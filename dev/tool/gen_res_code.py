...
# 名称: 自动生成资源代码
# 文件: 任意
# 作用域: $ProjectFileDir$/res [排除 res/icon]
# 程序: $ProjectFileDir$/.venv/Scripts/python.exe
# 实参: $ProjectFileDir$/dev/tool/gen_res_code.py
# 要刷新的路径: $ProjectFileDir$/src/config/res_config.py
"""
...
def generate_res_code(tree: dict, indent: int = 4) -> list[str]:
    ...
    for k, v in sorted(tree.items()):
        if isinstance(v, dict):
            lines.append(f"{ind}class {k}:")
            if not v:
                lines.append(f"{ind}    ...")
            else:
                lines.extend(generate_res_code(v, indent + 4))
        else:
            lines.append(f"{ind}{k} = _dir / '{v}'")
...
def main():
    ...
    res_out_file = proj_dir / 'src' / 'config' / 'res_config.py'

    tree = build_tree(res_dir)
